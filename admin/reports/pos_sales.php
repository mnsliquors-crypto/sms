...
$qry = $conn->query("
    SELECT t.*,
           ...
           0 as vat_amt,
           COALESCE((SELECT SUM(quantity) FROM transaction_items WHERE transaction_id = t.id), 0) as item_count,
...
$accounts_qry = $conn->query("SELECT * FROM account_list WHERE status=1 ORDER BY name ASC");
$acc_totals = [];
while($a = $accounts_qry->fetch_assoc()){
    $acc_totals[$a['id']] = ['name' => $a['name'], 'total' => 0];
}

// Fetch totals from transaction_list for POS payments in range
$acc_stmt = $conn->prepare("
    SELECT tl.account_id, SUM(tl.amount) as total 
    FROM transaction_list tl 
    JOIN transactions t ON tl.ref_id = t.id AND tl.ref_table = 'transactions'
    WHERE t.type = 'payment' 
    AND t.remarks LIKE '%[POS Payment]%'
    AND DATE(t.transaction_date) BETWEEN ? AND ?
    GROUP BY tl.account_id
");
$acc_stmt->bind_param("ss", $from, $to);
$acc_stmt->execute();
$acc_res = $acc_stmt->get_result();
while($aq = $acc_res->fetch_assoc()){
    if(isset($acc_totals[$aq['account_id']])){
        $acc_totals[$aq['account_id']]['total'] = $aq['total'];
    }
}
$acc_stmt->close();
?>
<div class="card card-outline card-success shadow">
    <div class="card-header">
        <h3 class="card-title"><i class="fas fa-cash-register mr-1"></i>POS Sales Report</h3>
        <div class="card-tools">
            <a href="<?php echo base_url ?>admin/reports/report_api.php?action=export_excel&report=pos_sales&from=<?php echo $from ?>&to=<?php echo $to ?>" class="btn btn-sm btn-success">
    ...
    <div class="card-body">
        ...
        <form method="GET" id="filter-form">
            <input type="hidden" name="page" value="reports/pos_sales">
            <div class="row align-items-end">
                ...
                <div class="form-group col-md-3">
                    ...
                    <a href="<?php echo base_url ?>admin/?page=reports/pos_sales" class="btn btn-sm btn-secondary"><i class="fas fa-times"></i> Clear</a>
                </div>
            </div>
        </form>
        <hr class="mt-0">
        ...
        <div id="print-data">
            <div class="text-center mb-2 d-none print-header">
                <h5>POS Sales Report</h5>
                <p class="mb-0"><?php echo date('d-m-Y',strtotime($from)) ?> to <?php echo date('d-m-Y',strtotime($to)) ?></p>
            </div>
            <div class="table-responsive">
                <table class="table table-bordered table-striped table-hover table-sm" id="posSalesTable">
                    ...
                    <tbody>
                    <?php if (empty($rows)): ?>
                        <tr><td colspan="12" class="text-center text-muted py-4">No POS records found for selected filters.</td></tr>
                    <?php endif; ?>
                    <?php $i=1; foreach ($rows as $row): ?>
                        <tr>
                            ...
                            <td class="text-center">
                                <?php $pm=$row['payment_method']??'Cash';
            ...
            <div class="row mt-3">
                <div class="col-md-6 offset-md-6">
                    <table class="table table-bordered table-sm">
                        <tr><th colspan="2" class="bg-dark text-white text-center">Payment Breakdown (POS)</th></tr>
                        <?php foreach($acc_totals as $at): if($at['total'] <= 0) continue; ?>
                        <tr>
                            <td><i class="fas fa-wallet text-secondary mr-2"></i> <?php echo htmlspecialchars($at['name']) ?></td>
                            <td class="text-right font-weight-bold"><?php echo number_format($at['total'],2) ?></td>
                        </tr>
                        <?php endforeach; ?>
                        <tr><td><i class="fas fa-credit-card text-danger"></i> Credit (Outstanding Balance)</td><td class="text-right font-weight-bold text-danger"><?php echo number_format($t_balance,2) ?></td></tr>
                        <tr class="table-dark"><th>Total POS Sales</th><th class="text-right"><?php echo number_format($t_net,2) ?></th></tr>
                    </table>
                </div>
            </div>
...
$(function(){
    ...
    $('.print-btn').click(function(){
        ...
        var _el = $('<div>');
        _h.find('title').text('POS Sales Report');
